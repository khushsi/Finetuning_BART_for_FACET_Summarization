...
def main(cfg: DictConfig) -> None:
    ...
    # if cfg.model.max_tokens  and cfg.model.max_tokens > shorter_pos_embeds.shape[0]:
    #     cfg.model.max_source_positions = cfg.model.max_tokens

    # if  cfg.model.max_tokens  and  cfg.model.max_tokens > shorter_pos_embeds.shape[0]:
    #     cfg.model.max_source_positions = cfg.model.max_tokens
    #     new_model = task.build_model(cfg.model)
    #
    #     correctly_shaped_pos_weight = new_model.encoder.embed_positions.weight.cuda()
    ...
    #     sd = model.state_dict()
    #     shorter_pos_embeds = sd['encoder.embed_positions.weight']
    #     print(shorter_pos_embeds.shape)
    ...
    #Code to consider for max_tokens more than default
    print(trainer.model.state_dict()['encoder.embed_positions.weight'].shape[0])
    ...
    #
        ...
        shorter_pos_embeds = sd['encoder.embed_positions.weight']
        # print(shorter_pos_embeds.shape)
        cfg.model.no_last_checkpoints = True
        trainer = Trainer(cfg, task, model, criterion, quantizer)
        sd2 = trainer.model.state_dict()
        # print(sd2['encoder.embed_positions.weight'].shape)

    logger.info(model)
    logger.info("task: {}".format(trainer.task.__class__.__name__))
    logger.info("model: {}".format(trainer.model.__class__.__name__))
    logger.info("criterion: {})".format(trainer.criterion.__class__.__name__))
    logger.info(
        "num. model params: {} (num. trained: {})".format(
            sum(p.numel() for p in model.parameters()),
            sum(p.numel() for p in model.parameters() if p.requires_grad),
